...
export const FormTable = createTable("form", {
  ...
  forkedFromId: integer("forked_from_id").notNull().references((): AnyPgColumn => FormTable.id),
...
export const FormVersionTable = createTable("form_version", {
  id: serial('id').primaryKey(),
  formId: integer("form_id").notNull().references(() => FormTable.id),
...
export const GroupInstanceTable = createTable("group_instance", {
  id: serial('id').primaryKey(),
  templateId: integer("template_id").notNull().references(() => GroupTemplateTable.id),
  formId: integer("form_id").notNull().references(() => FormTable.id),
...
export const PageInstanceTable = createTable("page_instance", {
  id: serial('id').primaryKey(),
  templateId: integer("template_id").notNull().references(() => PageTemplateTable.id),
  groupInstanceId: integer("group_instance_id").notNull().references(() => GroupInstanceTable.id),
  orderIndex: integer("order_index").notNull(),
  titleOverride: text("title_override"),
  descriptionOverride: text("description_override"),
  propertiesOverride: jsonb("properties_override").$type<Record<string, any>>().default({}),
...
export const ElementInstanceTable = createTable("element_instance", {
  id: serial('id').primaryKey(),
  templateId: integer("template_id").notNull().references(() => ElementTemplateTable.id),
  pageInstanceId: integer("page_instance_id").notNull().references(() => PageInstanceTable.id),
  orderIndex: integer("order_index").notNull(),
